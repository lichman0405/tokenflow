...
// It can be configured via the HYPER_BASE_URL environment variable.
func BaseURL() string {
	if url := os.Getenv("HYPER_BASE_URL"); url != "" {
		return url
	}
	return "https://api.hyper.example.com"
}
...
// The onSuccess callback is called with the user ID when authorization succeeds.
func PollForToken(ctx context.Context, deviceCode string, expiresIn int, onSuccess func(userID string)) (string, error) {
